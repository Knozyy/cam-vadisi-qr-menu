 ...
 */
export function LangStrip({ className = '' }) {
  const { lang, setLang } = useLang();

  return (
    <div className={`grid grid-cols-4 gap-1.5 ${className}`} role="group" aria-label="Dil seçimi">
      {LANG_ORDER.map((code) => {
        ...
        return (
          <button
            ...
            className={`flex min-w-0 flex-col items-center gap-1 rounded-lg px-1 py-1.5 text-[11px] font-semibold leading-none transition-colors ${
              active ? 'bg-sage text-pine' : 'bg-white/10 text-sage/80'
            }`}
          >
            <span
